...
function feedback(q,selected){
 const audit=q.auditStatus?`<h4>監査状態</h4><dl class="review-matrix">${Object.entries(q.auditStatus).map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`).join('')}</dl>`:'';
 const ok=correct(q,selected);
 const label=q.neutral?'採点対象外です':ok?'正解です':`正解は ${Array.isArray(q.answer)?q.answer.map(v=>v+1).join(' または '):q.answer+1} です`;
 const legal=q.currentLegalReview?`<h4>現行法令・通知の最終照合</h4><dl class="review-matrix">${Object.entries(q.currentLegalReview).map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`).join('')}</dl><p><a href="${esc(q.currentLegalUrl)}" target="_blank" rel="noopener">${esc(q.currentLegalSource)}</a></p>`:'';
 const review=q.structuredReview?`<h4>確認の観点</h4><dl class="review-matrix">${Object.entries(q.structuredReview).map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`).join('')}</dl>`:'';
 const choiceNotes=(q.verifiedChoiceExplanations||[]).length?`<h4>監修済み選択肢別解説</h4><ol>${q.verifiedChoiceExplanations.map(x=>`<li>${esc(x)}</li>`).join('')}</ol>`:'';
 const pending=!(q.verifiedChoiceExplanations||[]).length?`<p class="explanation-pending">選択肢ごとの詳細理由は監修待ちです。根拠未確認の説明は表示していません。</p>`:'';
 const current=q.currentSourceUrl?`<p>現行資料：<a href="${esc(q.currentSourceUrl)}" target="_blank" rel="noopener">${esc(q.currentSourceTitle)}</a></p>`:'';
 const examNote=q.examNote?`<h4>原試験の注意事項</h4><p>${esc(q.examNote)}</p>`:'';
 const answerAudit=q.answerAuditNote?`<h4>正答監査</h4><p>${esc(q.answerAuditNote)}</p>`:'';
 return `<section class="feedback"><h3>${label}</h3><p>${esc(q.explanation)}</p>${examNote}${answerAudit}${pending}${audit}${legal}${review}${choiceNotes}<p>根拠状態：${esc(q.evidenceStatus)}</p><p>正答監査：${esc(q.answerAuditStatus||'未実施')}</p><p>公開資料基準日：${esc(q.reviewDate||'2026-08-01')}／<a href="${esc(q.officialSourceUrl||'https://www.rbc.or.jp/exam/past_question/')}" target="_blank" rel="noopener">公式過去問</a></p>${current}</section>`;
}
